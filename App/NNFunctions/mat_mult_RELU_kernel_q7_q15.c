...
q7_t* mat_mult_relu_kernel_q7_q15(
   ...
   for (int i=0;i<ch_im_out; i+=2) {
     ...
     *pOut++ = sum>0 ? (q7_t) __SSAT((sum>>   out_shift),8):0;
     *pOut++ = sum3>0 ? (q7_t) __SSAT((sum3>> out_shift),8):0;
     *pOut2++ = sum2>0 ? (q7_t) __SSAT((sum2>>out_shift),8):0;
     *pOut2++ = sum4>0 ? (q7_t) __SSAT((sum4>>out_shift),8):0;
   ...
   // return the output pointer with offset
   return pOut;
}
